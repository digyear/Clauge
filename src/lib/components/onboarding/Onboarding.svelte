...
{#if show}
<div class="ob-overlay" class:visible={mounted} onmousedown={handleOverlayMousedown}>
  {#if showCustomChrome}
    <div class="ob-wc" data-drag-region>
      <button class="ob-dot ob-close" onclick={wcClose} aria-label="Close"></button>
      <button class="ob-dot ob-min" onclick={wcMinimize} aria-label="Minimize"></button>
      <button class="ob-dot ob-max" onclick={wcFullscreen} aria-label="Fullscreen"></button>
    </div>
  {/if}

  <div class="ob-card">
    <img src="/clauge-icon-animated.svg" alt="Clauge" class="ob-logo" width="72" height="72" />
    ...
    <div class="ob-btns">
      {#if ghConnecting}
        <div class="ob-waiting">
          ...
          <span class="ob-waiting-txt">Waiting for authorization…</span>
          <button class="ob-cancel" onclick={() => (ghConnecting = false)}>Cancel</button>
        </div>
      {:else}
...
<style>
  ...
  .ob-dot {
    ...
    border: none; cursor: default; padding: 0; transition: filter 0.1s;
  }
  .ob-dot:hover { filter: brightness(0.85); }
  .ob-close { background: #ff5f57; }
  .ob-min   { background: #febc2e; }
  .ob-max   { background: #28c840; }
  :global(body.window-blurred) .ob-dot { background: var(--t4) !important; }
  ...
  .ob-title {
    ...
    color: var(--t1); font-family: var(--ui);
  ...
  .ob-sub {
    ...
    font-family: var(--ui); margin: 0 0 12px;
  ...
  .ob-btn-gh:hover { background: #f0f0f0; }
  .ob-btn-gh:active { transform: scale(0.98); }

  .ob-btn-google {
    display: flex; align-items: center; justify-content: center; gap: 10px;
    height: 44px; border-radius: 10px;
    background: transparent; color: var(--t3);
    border: 1px solid var(--b1); font-size: 13px; font-weight: 500;
    font-family: var(--ui); cursor: not-allowed;
    opacity: 0.5; width: 100%; position: relative;
  }
  ...
  .ob-waiting-txt {
    font-size: 13px; color: var(--t3); font-family: var(--ui);
  }
  .ob-cancel {
  ...
  .ob-cancel:hover { background: var(--b1); }
